...
: 자바에서 DB에 접근하기 위해(연결하기 위해) 제공하는 표준 API

" Java 애플리케이션 -> SQL 실행 -> DB 결과를 받아오기 "
MySQL, Oracle, MariaDB 등 다양한 DB와 Java를 연동
데이터베이스와 연결, 질의, 결과, 종료를 담당

1) JDBC의 역할
: DB의 접속하고 SQL 문을 작성하고 전송, SQL 결과를 Java의 객체 형태로 받아옴, DB 연결 종료 및 자원 해제

2) JDBC의 필요성
: Java 에서는 SQL 문장을 직접 실행 X
- DB 드라이버와 JDBC API를 통해 DB와 연결
- Java 에서 표준화 된 SQL을 사용하여 DBMS의 유연성을 확보 (유지보수, 효율성)
    >> 통합된 표준 인터페이스 제공
...
>> [SQL 작성 및 실행] >> [결과처리(ResultSet)] >> [지원 해제(Connection, Statement, ResultSet close)]
